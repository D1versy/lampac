...
namespace JacRed.Engine
{
    ...
    ///  1. **Хост узла берём из адреса запроса, а не из тела.** Узел не может ошибиться в
    ///     собственном IP (за NAT он его и не знает), а подделать источник в обход туннеля нельзя.
    ///  2. **Живость НЕ проверяем своей пробой.** Агент на узле объявляет только рабочие роли, а
    ///     реальный отказ ловит ok()-предикат трекера + ProxyFallback. Вторая, более тупая система
    ///     проверок врала бы — ровно этим кончились активные хелс-чеки (боевой лог §BR).
    ...
    public static class NodeRegistry
    {
        ...
        /// </summary>
        /// <param name="ip">адрес ИСТОЧНИКА запроса, не из тела</param>
