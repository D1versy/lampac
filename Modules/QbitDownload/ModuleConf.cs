...
public class ModuleConf : ModuleBaseConf
{
    ...
    // d1v:music-player-server — какой плеер использует раздел «Музыка». Решение СЕРВЕРА, а не
    // устройства.
    //
    // Зачем. Выбор жил в localStorage каждого клиента (`lampac_music_player`), и починка плеера
    // означала «зайди в настройки на каждом устройстве». Владелец: «сделай так, чтобы это всё
    // настраивалось на сервере, а не на клиенте». Значение едет клиентам в /d1vision/hosts.json
    // рядом с brand — то есть меняется правкой init.conf на лету, без пересборки и без
    // прикосновения к устройствам, и откатывается так же быстро.
    //
    // Допустимые значения: "inner" — встроенный видеоплеер Lampa (его панель несёт перемотку,
    // качество, дорожки и субтитры — для музыки это лишнее); "ios" — собственный <audio> с
    // четырьмя кнопками (пред / пауза / след / плейлист). Пустая строка — не навязывать,
    ...
    // ⚠️ Прогресс здесь СПРАВОЧНЫЙ — только для первой отрисовки грида. Живую свежесть с
    // qdl 2.93 диктует отдельная лёгкая ручка /qdl/progress (Progress.cs), поэтому ронять
    // этот TTL ради процентов не нужно: это вернуло бы секунду пересборки на горячий путь.
    public int listCacheSeconds { get; set; } = 30;

    // ── Живой прогресс загрузок (qdl 2.93, Progress.cs + поллер в qdl.js) ──────
    // Интервал опроса /qdl/progress, пока что-то реально качается.
    // 0 = ВЫКЛЮЧИТЬ целиком: ручка отвечает ok:false не трогая qBittorrent, клиент таймер
    // не заводит и падает на прежнее поведение (прогресс из /qdl/list). Киллсвитч на лету.
    public int progressPollSeconds { get; set; } = 5;

    // Снимок ответа qBittorrent внутри ручки — коалесит несколько устройств в один поход.
    // 0 = без снимка (каждый тик каждого устройства идёт в qBit).
    public int progressSnapshotSeconds { get; set; } = 2;

    // Медленный пульс, когда недокачанное есть, но НЕ движется (нет пиров / в очереди / пауза).
    // 0 = не опрашивать вовсе — буквальный режим «только пока идут активные загрузки»
    // (тогда возобновившаяся раздача разблокирует серию лишь при следующем входе на экран).
    public int progressIdlePollSeconds { get; set; } = 30;

    // Сколько минут держать медленный пульс, прежде чем замолчать совсем. 0 = без ограничения.
    // Нужен ровно затем, чтобы одна вечно стоящая раздача не держала опрос всю сессию.
    public int progressIdleBudgetMinutes { get; set; } = 10;

    // Жёсткая блокировка воспроизведения недокачанного (решение владельца 01.09.2026).
    // false = киллсвитч на лету: возвращается прежнее предупреждение с «смотреть всё равно».
    // ⚠️ Страховка под жёсткий гейт: если он где-то ошибётся, зритель не должен остаться
    // запертым до пересборки образа.
    public bool partialPlayBlock { get; set; } = true;

    // ── Поиск (qdl 2.121): контекстная кнопка шапки + свой экран поиска «как в XSMART» ──
    // Кнопка поиска в шапке (и нижняя панель телефона, и голосовой запрос Android) ведёт туда,
    // где зритель находится: в XSMART — поиск XSMART, в jut.su — поиск jut.su, везде остальное —
    // наш экран d1v_search (поле сверху, «Недавнее», выдача сеткой; на ТВ полноэкранная
    // клавиатура Lampa). false = киллсвитч на лету: все входы снова открывают штатный оверлей
    // Lampa; экраны jut.su/XSMART при этом остаются, меняется только маршрут. Едет клиенту ключом
    // search.screen в /qdl/features (loadFeatures раз в 60 с). Канон — медиасервер claude/19-search.md.
    public bool searchScreen { get; set; } = true;
    ...
    public int bitmagnetLimit { get; set; } = 100;         // интерактив: top-N по сидам на тайтл
    public int bitmagnetTimeoutSec { get; set; } = 8;
    // ⚠️ Устарело с qdl 2.107 и больше не читается. Деление сидов пополам смотрело на
    // torrent_contents.updated_at, а он двигается при ПЕРЕКЛАССИФИКАЦИИ контента (и заодно
    // обнуляет копию сидов), а не при пересканировании роя — то есть штраф ложился ровно на
    // строки с честными числами. Сиды теперь берутся как greatest(tc.seeders, dht.seeders) и
    // в охоте служат только ключом порядка (sid_hint), живость доказывает проба метаданных.
    // Ключ оставлен, чтобы не ломать существующий init.conf.
    public int bitmagnetStaleSeedsDays { get; set; } = 3;
    // Интерактивная выборка = top-N по сидам ∪ top-M по дате появления в DHT (t.created_at).
    // Без второй ветки свежая серия у сериала с тысячей привязанных раздач не попадала в сотню
    // (у «Укрытия» S03E10 стояла на 208-м и 362-м местах по сидам). 0 = только по сидам, как было.
    public int bitmagnetFreshLimit { get; set; } = 100;
    // Охотничья выборка: все раздачи СЕЗОНА по episodes jsonb (285–610 строк на сезон у наших
    // сериалов, 20–270 мс; самый крупный сезон базы — 1101 строка, 0.7 с). Потолок страховочный,
    // окно по свежести; срез виден в логе и dry-run (bitmagnetTruncated).
    public int bitmagnetHuntLimit { get; set; } = 3000;
    ...
    // Локальный тик охоты (qdl 2.107): каждые N минут — ТОЛЬКО bitmagnet + наш индекс, без трекеров.
    // Четырёхчасовой интервал выше существует ради трекеров; локальный Postgres можно спрашивать
    // хоть каждые 10 минут. Против спама: только сериалы с недостающими сериями, детектор изменений
    // (нет новых строк → ни скоринга, ни лога), пробы только новых кандидатов под бэкоффом blacklist,
    // без апгрейдов и без сигнала re-grab, одна сводка в лог раз в huntLocalSummaryMinutes.
    // Читается на лету. 0 = выключен.
    public int huntLocalIntervalMinutes { get; set; } = 10;
    public int huntLocalSummaryMinutes { get; set; } = 60;
    // Охота ходит в bitmagnet по TMDB id записи + сезон (episodes jsonb). false = как до 2.107:
    // без tmdb_id, DHT-строки доезжают только эхом нашего индекса.
    public bool huntBitmagnet { get; set; } = true;
    public int donorMinSeeds { get; set; } = 3;             // только для трекерных строк: у bitmagnet сиды — подсказка (sid_hint)
    // Порог качества донора. 720, а не 1080 (решение владельца 2026-09-04): «лучшее из доступного,
    // но не ниже 720p». 0 = порог выключен — остальные гейты качества (ниже) он НЕ трогает.
    public int donorMinQuality { get; set; } = 720;
    // 🔴 Смена политики (2026-09-04): quality==0 («не распознано») раньше ПРОХОДИЛ гейт качества —
    // так донором «Укрытия» стал 720×400 XviD с kinozal («…/ WEBRip» без цифры). Теперь неизвестное
    // качество = ниже порога. У multi-раздач bitmagnet качество перед вердиктом добирается из имён
    // файлов (DominantQuality), у трекерных паков без токена разрешения — нет, они отсеиваются.
    public bool donorRejectUnknownQuality { get; set; } = true;
    public bool donorRejectLegacy { get; set; } = true;      // XviD/DivX/MPEG4/MPEG2 — отсев независимо от порога
    public bool donorRejectScreener { get; set; } = true;    // CAM/TELESYNC/TELECINE/WORKPRINT/SCREENER — отсев независимо от порога
    // Донор — только с признаками русской дорожки (TorrentScoring.IsRussian: кириллица / маркеры
    // студий / lang_ru). Английские, китайские, украинские поштучные серии из DHT не рассматриваются.
    public bool donorRequireRussian { get; set; } = true;
    // Целевое качество донора: 0 = как у основной раздачи (по именам её файлов; ниже 1080 → 1080),
    // иначе явное значение. Ранг: ровно цель → выше цели (ближе — лучше) → ниже цели (выше — лучше).
    public int donorQualityTarget { get; set; } = 0;
    ...
    // Доноры охоты дом → реплика (ReplicaDonors.cs, qdl 2.116.1): временные серии с чужих раздач едут
    // на tv2 приложением к своей основной (нужный файл в донорской категории, как дома) и снимаются,
    // когда дом их снял. false = киллсвитч: ни добора, ни снятия, уже добранные остаются как есть.
    public bool replicaDonors { get; set; } = true;
