...
## Вкладки «Решения» и «Уведомления» в админке (qdl 2.111)
...
| Маршрут | Назначение |
|---|---|
| `GET /admin/d1v/api/decisions` | вкладка «Решения»: открытые предложения смены раздачи (`pendingSwitch` из `watch.json`) |
| `POST /admin/d1v/api/events/switch` | `{hash, accept}` — принять/отклонить смену раздачи |
| `GET /admin/d1v/api/events` | вкладка «Уведомления»: журнал + строки ленты, последние 100 (зрителю — 50) |
| `POST /admin/d1v/api/events/clear` | очистить журнал (ленту зрителей не трогает) |

🔴 `/api/events` **сливает** `events.json` со строками таблицы `noti`: журнал копится с момента
выката, и без слияния вкладка пуста первые дни. Зеркала `cat=user` в журнал не пишутся — источник
правды по ним сама `noti`. Старые служебные строки в `noti` раскладываются по настоящим
категориям (`NotiCat`), а не выдаются за увиденное зрителем.
...
## Преемник раздачи (`Successor.cs`, qdl 2.115)

Re-grab перевыложенного топика и переключение на более полную раздачу больше не снимают старый торрент сразу. Watch-запись держит `hash` = старая раздача и поле `next = {hash, reason, link, mode, savepath, since, deadline}`; преемник живёт в категории `lampa` с тегом `qdl-next` и **всегда добавляется в свою подпапку `/downloads/.next/<hash8>`**, старая ставится на стоп, но играется. Режим по спискам файлов: `flat` (совпадений путей нет или те же размеры → переезд в общий корень при нуле скачанных байт, перепроверка засчитывает старые файлы) или `aside` (тот же путь, другой размер → остаётся в подпапке, старая не тронута). Зритель: преемник скрыт из `/qdl/list`, его серии — в `/qdl/episodes` источником `next`, прогресс по своему хешу. Жнец `ScanSuccessors` (15-минутный тик) снимает старую, когда каждая её докачанная серия есть у преемника (или преемник готов целиком): файлы старой удаляются только в `aside` и только по safe-пути; по сроку `successorMaxDays` — принудительная замена, файлы остаются. `/qdl/delete` и `/qdl/watch/remove` отменяют замену, `/qdl/transcode` при замене отказывает, охота и `ConsiderSwitch` по записи стоят. Ход замены — во вкладке «Решения» админки (`successors` на `/admin/d1v/api/decisions`) и в строке хелса «Замена раздач (слежение)». Киллсвитч `successorEnabled`. Разбор — медиасервер `claude/06` §DR.
