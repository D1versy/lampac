...
public class ModuleConf : ModuleBaseConf
{
    ...
    // Реплике сидировать не нужно, а раздача с датацентрового адреса — это abuse-письма.
    // Символический лимит отдачи (КБ/с) выставляется в qBittorrent реплики при добавлении.
    ...
    // Тормоз массовости: сирот больше N% нашего набора — проход не выполняется ЦЕЛИКОМ.
    // Аналог replicaShrinkGuardPercent, но по другому основанию: shrink сравнивает СЧЁТЧИК
    // манифеста с прошлым тиком, а этот — ПЕРЕСЕЧЕНИЕ нашего набора с домашним. Ловит
    // валидный, но не тот снимок дома, который shrink пропустил бы.
