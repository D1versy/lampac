...
lampainit_invc.appload = function appload() {
  ...
  // тихо падает, пункт меню не появляется, остальная Lampa работает.
  // Развилка адреса — ровно та же, что у XSMART выше: дома контейнер на своём порту 9141,
  // снаружи /sounds/* разводит Caddy на том же origin. 🔴 Порт 9141 обязан быть разрешён в CSP
  // ({sounds} в D1VContentPolicy.cs): 'self' сверяет и ПОРТ, и без этого браузер режет тег
  // молча — на экране это выглядит как «пункт меню просто не появился».
  (function () {
    var L = '{localhost}';
    var lan = /^https?:\/\/(localhost|127\.0\.0\.1|\[?::1\]?|10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/i.test(L);
    var base = lan ? L.replace(/:\d+$/, '') + ':9141' : L;
    Lampa.Utils.putScriptAsync([base + '/sounds/sounds.js?v={version}']);
  })();
