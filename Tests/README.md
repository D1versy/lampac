...
## Что добавилось 25.08.2026

- **Право «Управление» (qdl 2.67)** — `ManageGateTests.cs`, 34 теста. Гейт `ManageDenied()`
  закрывает семь мутирующих ручек (`/qdl/delete`, `/qdl/transcode`, пять `/qdl/collections/*`),
  которые до 2.67 были открыты всем, кто дотянулся до `/qdl`. Покрыто: фича в белом списке
  `Perms.Features`, независимость грантов (`live`/`rec` не открывают `manage` и наоборот),
  строгое сравнение куки владельца `qdl_unlock=1` (мусорные варианты ключом не являются),
  киллсвитч `permsEnabled:false` (fail-open), «с правом запрос доходит до валидации аргументов»
  (гейт не съедает тех, кому можно), и то, что чтение (`/qdl/list`, `/qdl/collections`) правом
  НЕ гейтится. 🔴 Отдельно — порядок гейтов: на реплике обязан отвечать `ReplicaReadOnlyDeny()`,
  а не гейт прав; проверяется и поведением, и сторожем по исходнику `Controller.cs`.
  Все 34 прогнаны мутационно (11 поломок продакшн-кода) — каждый краснеет хотя бы на одной.
