...
public static class Perms
{
    ...
    // (шестерёнка «Настройки», пункт «Консоль», экран «Хелс-чеки», а с 2.89 ещё и раздел
    // настроек «D1Vision» с глобальным фильтром каталога). До 2.67 всё перечисленное открывала
    // ТОЛЬКО кука qdl_unlock=1, которой у приложений нет и быть не может, — то есть из
    // mac/iOS/Android/Windows эти действия были недоступны в принципе. Теперь их выдают устройству
    // галочкой в /admin/d1v.
    // 🔴 qdl 2.89: кука убрана совсем (решение владельца) — единственный ключ теперь это право.
    //
...
public partial class QbitController
{
    ...
    #region гейт «Управления» (qdl 2.67, кука убрана в 2.89)
    ...
    ///
    /// 🔴 qdl 2.89: второго ключа больше нет. Кука qdl_unlock=1 (мастер-ключ владельца из 2.39)
    /// убрана по его решению — «всё завязано по пермишину через админку». Страховкой от
    /// самозапирания при потере access.json остаётся сама админка /admin/d1v: она открывается
    /// кукой accspasswd (рут-пароль из D:\docker\config\lampac\passwd), от прав устройств не
    /// зависит и переживает потерю реестра. Аварийный рубильник на всё — permsEnabled:false.
    /// </summary>
