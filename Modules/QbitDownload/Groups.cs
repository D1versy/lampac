...
public static class Groups
{
    ...
    /// <summary>
    /// 🔴 «Ноль не затирает прогресс». Плеер в момент ОТКРЫТИЯ карточки шлёт запись
    /// {"duration":0,"time":0,"percent":0} — такие строки лежат в боевой базе прямо сейчас.
    /// У одного устройства это безобидно, а в группе третий, просто открывший тайтл, обнулял бы
    /// позицию первым двум. Правило: запись с time&lt;=0 или duration&lt;=0 не затирает строку,
    /// у которой time&gt;0.
    ///
    /// Осознанный пересмотр с начала этим не ломается: как только пойдёт настоящее
    /// воспроизведение, time станет больше нуля и запись пройдёт.
    ///
    /// Цена: один точечный SELECT по уникальному индексу и ТОЛЬКО когда входящая запись нулевая.
    /// В обычном потоке (ненулевой прогресс) БД здесь не трогается вовсе.
    /// </summary>
    ...
    /// <summary>Запись «ничего не воспроизводилось»: нет длительности или нет позиции.</summary>
    internal static bool IsZeroRoad(string data)
    {
        try
        {
            ...
            double duration = o.Value<double?>("duration") ?? 0;
