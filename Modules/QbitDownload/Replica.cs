...
public partial class QbitController
{
    ...
    async public Task<ActionResult> ReplicaManifest()
    {
        ...
        var manifest = new JObject
        {
            ...
            ["notiSig"] = NotiSigSafe(),
            // qdl 2.90: глобальная настройка фильтра каталога по году едет из дома.
            // 🔴 Решение владельца: «пусть это всё тянется с нашего основного сервера». На реплике
            // нет НИ ОДНОГО устройства с правом «действия», то есть выставить её там через UI
