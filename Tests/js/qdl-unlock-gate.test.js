'use strict';
// qdl 2.39: скрытый функционал (транскод / удаление с файлами / экран «Хелс-чеки»)
// открывается кукой qdl_unlock=1. Без неё пункты не рисуются НИКОМУ — ни приложениям,
// ни браузеру: платформенных веток тут сознательно нет, кука и есть единственный ключ.
...
test('quickMenu без куки: ни транскода, ни удаления, но обычные пункты на месте', () => {
  ...
  assert.ok(titles.some((t) => t.indexOf('коллекци') !== -1), 'коллекции остаются');
});
