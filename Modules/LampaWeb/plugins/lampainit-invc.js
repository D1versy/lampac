...
// Нативные оболочки добавляют в UA токен "d1vision_<platform>/<версия>" (mac|ios|tvos|android|tizen|windows);
...
// живут на андроидной ветке запуска плеера, поэтому mac/ios/tvos/android/windows получают одинаковые
...
try {
  ...
  var d1tok = /d1vision_(mac|ios|tvos|android|tizen|windows)\/(\S+)/i.exec(d1ua);
  ...
  if (d1plat === 'mac' || d1plat === 'ios' || d1plat === 'tvos' || d1plat === 'android' || d1plat === 'windows') {
