...
function loadQdl(opts) {
  ...
  // qdl 2.39: транскод/удаление/хелс-чеки гейтятся кукой qdl_unlock=1 (qdlUnlocked()).
  // Дефолт тест-окружения — «разблокировано», чтобы старые тесты quickMenu видели все пункты;
  // состояние «без куки» тестируется явно через opts.cookie: ''.
  const doc = opts.document || makeDocument();
  if (opts.cookie !== undefined) doc.cookie = opts.cookie;
  else if (doc.cookie === undefined) doc.cookie = 'qdl_unlock=1';
  const sandbox = {
    window: win,
...
function loadQdlDom(opts) {
  ...
  const w = dom.window;
  if (opts.cookie === undefined) w.document.cookie = 'qdl_unlock=1';   // дефолт «разблокировано» (см. loadQdl)
  else if (opts.cookie) w.document.cookie = opts.cookie;
