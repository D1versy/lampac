...
## Вкладка «Уведомления» в админке (qdl 2.111)

Зритель видит в ленте только «вышла новая серия/сезон» и итог большой пачки; вся механика
(постановка в очередь, смена и перекачка раздач, охота, качество, диагностика) уходит в журнал
владельца — кольцо `<cachePath>/events.json` (`EventLog.cs`, кэп `adminEventsKeep`).
Маршрутизация видов и формулировки — `NotiRoute.cs`, ОДНО место на все три контура.

| Маршрут | Назначение |
|---|---|
| `GET /admin/d1v/api/events` | журнал + `pending` (открытые предложения смены раздачи из `watch.json`) |
| `POST /admin/d1v/api/events/switch` | `{hash, accept}` — принять/отклонить смену раздачи |
| `POST /admin/d1v/api/events/clear` | очистить журнал (ленту зрителей не трогает) |

🔴 Новую таблицу в SQLite завести нельзя (`EnsureCreated()` не добавляет её в существующую БД),
поэтому журнал — JSON, а аудитория строки определяется по `kind` в `NotiRoute`. Дедуп
демотированных видов живёт в `seen` (её не чистит `NotiPrune`). Разбор — `claude/06` §DN.
